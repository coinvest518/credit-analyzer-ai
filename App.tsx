...
Return ONLY valid JSON.`;
  ...
  const handleGenerateReport = async () => {
    ...
    try {
        ...
        setSummaryReport(response.choices[0].message.content!);
  ...
  const handleGenerateActionPlan = async () => {
    ...
    try {
        ...
        const response = await client.chat.complete({ model: 'mistral-large-latest', messages: [{ role: 'user', content: prompt }] });
        setActionPlan(response.choices[0].message.content!);
  ...
  const handleGenerateLetterPackage = async () => {
    ...
    try {
        ...
        generated.creditBureau = response.choices[0].message.content!;
        ...
        generated.debtCollector = response.choices[0].message.content!;
        ...
        response = await client.chat.complete({ model: 'mistral-large-latest', messages: [{ role: 'user', content: prompt }] });
        generated.creditor = response.choices[0].message.content!;
        ...
        response = await client.chat.complete({ model: 'mistral-large-latest', messages: [{ role: 'user', content: prompt }] });
        generated.cfpb = response.choices[0].message.content!;
